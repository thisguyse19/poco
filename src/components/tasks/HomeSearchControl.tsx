...
export function HomeSearchControl({ open, query, onOpenChange, onQueryChange }: Props) {
  ...
  return (
    <div ref={wrapRef} className="pointer-events-none flex w-full justify-end">
      <div
        className={`pointer-events-auto flex max-w-full flex-row-reverse items-stretch overflow-hidden rounded-none border border-[var(--border-default)] bg-[var(--bg-elevated)] shadow-sm transition-[max-width] duration-[400ms] [transition-timing-function:var(--ease-ios)] ${
          open ? 'w-full max-w-full' : 'w-10 max-w-[2.5rem]'
        }`}
      >
        <button
          type="button"
        ...
        <input
          ...
          className={`poco-input h-10 min-w-0 flex-1 rounded-none border-0 bg-transparent text-sm outline-none transition-[opacity,padding,max-width] duration-[400ms] [transition-timing-function:var(--ease-ios)] ${
            open ? 'max-w-full px-3 opacity-100' : 'max-w-0 px-0 opacity-0'
          }`}
          tabIndex={open ? 0 : -1}
          aria-hidden={!open}
        />
